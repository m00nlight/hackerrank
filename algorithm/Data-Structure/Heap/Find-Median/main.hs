...
queryMedian (small, big)
    ...
    | getSize small >  getSize big           =
        printf "%.1f" ((fromIntegral $ msGetMax small )/ (1 :: Double))
    | getSize small <  getSize big           =
        printf "%.1f" ((fromIntegral $ msGetMin big) / (1 :: Double))
    | otherwise                              =
        let s1 = msGetMax small
            s2 = msGetMin big
        in printf "%.1f" (fromIntegral (s1 + s2) / 2 :: Double)
...
main = do
  ...
  let qs  = map readInt' content
      res = solve $ map (\ x -> ("a", x)) qs
  BS.putStrLn $ BS.pack (L.intercalate "\n" res)
